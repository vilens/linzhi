...
juicer.register("fixed2",function(value){
    if(!isNaN(value)) {
...
juicer.register("cutstring",function(str,len) {
    // length属性读出来的汉字长度为1
    len= len|| 10;
    if (str.length * 2 <= len) {
        return str;
...
juicer.register("dateFormat",function(time, fmt) {
    if(!time){
        return "";
    }
    ...
    var o = {
        ...
        "d+" : time.getDate(), // "+getI18n("public.common.date")+"
        "h+" : time.getHours(), // 小时
        "m+" : time.getMinutes(), // 分
        "s+" : time.getSeconds(), // "+getI18n("public.common.second")+"
...
juicer.register("showSex",function(sex){
   var  showSexText="";
    sex = parseInt(sex);
    switch(sex){
        case 0:
