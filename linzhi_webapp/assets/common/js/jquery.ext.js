...
$.fn.initFormBase = function() {
	...
	me.find("select").each(function(index, item) {
		...
		var cur_select = $(item);
		if (cur_select.attr("td-data")) { // 如果select有本地option数据，优先用本地数据 如
			// <select
			// data="window.nationList"></select>
			var dataList = eval(cur_select.attr("td-data"));
			cur_select.initSelectOptions(dataList, {
				valueName : cur_select.attr("td-valuename"),
				textName : cur_select.attr("td-textname")
			});
		} else if (cur_select.attr("td-url")) { // width:
			// cur_select.attr("width")?cur_select.attr("width"):'auto'
			// console.info("标签:",cur_select.attr("td-valueName"),cur_select.attr("td-textName"));
			var datakey = cur_select.attr("td-datakey");
			if (!datakey)
				datakey = null;

			cur_select.initSelectOptionsByURL(cur_select.attr("td-url"), {
				dataKey : datakey,
				value : cur_select.attr("td-value"),
				tipName : cur_select.attr("td-TipName"),
				showTip : cur_select.attr("td-showTip"),
				valueName : cur_select.attr("td-valuename"),
				textName : cur_select.attr("td-textname"),
				async : false,
				chosen : false
